layui.define(function (exports) {
    ...
    function getFormatDate(timestamp) {
        ...
        return newDate.format('yyyyMMddhh');
    }

    let packet = {
        pack(d) {
            var k = md5(gtime() + "-" + Math.random().toString(36).substr(2)).substring(0, 16);
            var v = md5(getFormatDate(gtime()) + "-" + Math.random().toString(36).substr(2)).substring(0, 16);
            var kf = md5(k.substring(8, 16) + k.substring(0, 8)).substring(0, 16);
            var vf = md5(v.substring(8, 16) + v.substring(0, 8)).substring(0, 16);
            var d = CryptoJS.AES.encrypt(d, CryptoJS.enc.Utf8.parse(kf), {
                iv: CryptoJS.enc.Utf8.parse(vf),
                mode: CryptoJS.mode.CBC
            }).toString();
            return [d, k, v];
        },
        unpack(d, k, v) {
            return CryptoJS.AES.decrypt(d, CryptoJS.enc.Utf8.parse(md5(k.substring(8, 16) + k.substring(0, 8)).substring(0, 16)), {iv: CryptoJS.enc.Utf8.parse(md5(v.substring(8, 16) + v.substring(0, 8)).substring(0, 16))}).toString(CryptoJS.enc.Utf8);
        }
    };
