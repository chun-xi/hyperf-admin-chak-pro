...
layui.define(['laytpl', 'layer', 'pack'], function (exports) {
    var $ = layui.jquery
        , laytpl = layui.laytpl
        , layer = layui.layer
        , setter = layui.setter
        , device = layui.device()
        , hint = layui.hint(), packet = layui.pack
     ...
     * @return {string}
     */
    view.JSONObjectToParams = function (obj) {
        let params = '';
        for (let key in obj) {
    ...
    view.req = function (options) {
        ...
        if (options.data instanceof Object) {
            options.data = view.JSONObjectToParams(options.data);
        }
        ...
        return $.ajax($.extend({
            ...
            , success: function (res) {
                ...
                if (typeof res._x != "string" || typeof res._s != "string" || typeof res._v != "string") {
                    view.error("数据包不完整");
                    return;
                }

                let unpack = packet.unpack(res._x, res._s, res._v);
                res = JSON.parse(unpack);
                console.log(res);
